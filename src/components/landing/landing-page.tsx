...
import { FaWhatsapp, FaFacebookMessenger, FaEnvelope, FaLinkedinIn, FaInstagram, FaYoutube } from "react-icons/fa";
...
import { AnimatedSecurityFeatures } from "./animated-security-features";
import { GlassmorphicFeatures } from "./glassmorphic-features";
import { CircularChannels } from "./circular-channels";
  ...
  return (
    ...
    >
      ...
      <section className="relative min-h-[calc(100vh-80px)] lg:min-h-[calc(100vh-96px)] flex items-center justify-center border-none py-12 lg:py-0">
        ...
        >
          <motion.div variants={fadeUp} className="flex flex-col items-center text-center lg:items-start lg:text-start">
            ...
            {(copy as any).heroPoints ? (
              <ul className="mt-8 flex flex-col gap-4 text-start">
                {(copy as any).heroPoints.map((point: any, i: number) => {
                  const icons = [Clock, Bot, Database, Layers];
                  const Icon = icons[i] || CheckCircle2;
                  return (
                    <li key={i} className="flex items-start gap-4">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#6119E6] text-white dark:bg-[#E13382]">
                        <Icon size={20} />
                      </div>
                      <p className="text-base font-medium text-slate-700 dark:text-slate-300 pt-2">
                        <strong className="font-extrabold text-slate-900 dark:text-white mr-1.5">{point.title}</strong>
              ...
              <p className="mt-6 max-w-2xl text-lg font-medium leading-8 text-slate-600 dark:text-slate-300 sm:text-xl">
                {(copy as any).subtitle}
              </p>
            )}

            <div className="mt-10 flex w-full max-w-sm flex-col items-stretch gap-3 sm:max-w-none sm:flex-row sm:flex-wrap sm:items-center sm:justify-center lg:justify-start">
              <Link
            ...
            <p className="mt-3 flex items-center justify-center lg:justify-start gap-1.5 text-sm font-medium text-slate-500 dark:text-slate-400">
              <CheckCircle2 size={16} className="text-emerald-500" />
              {copy.heroButtons?.trySub}
            </p>
          </motion.div>

          <motion.div variants={fadeRight} className="relative mx-auto w-full max-w-xl sm:max-w-2xl lg:mx-0">
            <Image
              src="/newhero.png"
        ...
        >
          <button
            ...
            className="flex h-12 w-12 items-center justify-center rounded-full bg-white text-slate-900 shadow-xl border border-slate-200 transition hover:bg-slate-50 hover:scale-110 dark:bg-white/10 dark:text-white dark:border-white/10 dark:hover:bg-white/20 animate-bounce"
      ...
      <section id="features" className="relative">
        <GlassmorphicFeatures 
          featuresTitle={copy.featuresTitle}
          featuresData={copy.features.map(([title, text]) => ({ title, text }))}
        />
      </section>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="mt-2 text-sm leading-7 text-slate-600 dark:text-secondary-100/80">
                          <FormattedText>{step.text}</FormattedText>
                        </p>
                      </div>
                    </motion.div>
                  );
      ...
      <section id="channels" className="relative z-10 py-20 sm:py-24">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-slate-600 dark:text-secondary-100/85">
      ...
      <section id="security" className="relative z-10 py-20 sm:py-24 bg-slate-50 dark:bg-transparent">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-extrabold leading-tight text-slate-950 dark:text-white sm:text-5xl">{copy.securityTitle}</h2>
            {copy.security && (
              <p className="mx-auto mt-6 max-w-3xl text-lg font-medium text-slate-600 dark:text-slate-300">
                <FormattedText>{copy.security}</FormattedText>
              </p>
            )}
          </div>
          <AnimatedSecurityFeatures points={copy.securityPoints || []} />
        </div>
      </section>
      ...
      <section id="contact" className="relative z-10 py-16 sm:py-20 border-none bg-slate-50 dark:bg-transparent">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-2 items-center">
            <div className="rounded-3xl border border-slate-200 bg-white/50 p-8 shadow-lg backdrop-blur-xl dark:border-white/5 dark:bg-[#0c081c]/50">
              <h3 className="text-2xl font-extrabold text-slate-950 dark:text-white mb-8 text-center">{isEnglish ? "Get in touch" : "تواصل معنا"}</h3>
              <div className="flex flex-col gap-6">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-bold text-slate-500 dark:text-slate-400">{isEnglish ? "Office" : "المكتب"}</p>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-bold text-slate-900 dark:text-white" dir="ltr">{copy.contact?.address}</p>
                  </div>
                </div>
              </div>
            </div>

            <div className="rounded-3xl border border-slate-200 bg-white/50 p-6 shadow-lg backdrop-blur-xl dark:border-white/5 dark:bg-[#0c081c]/50 flex flex-col justify-center">
              <h3 className="text-lg font-extrabold text-slate-950 dark:text-white mb-4 text-center">{isEnglish ? "Follow us" : "تابعنا"}</h3>
              <div className="flex items-center justify-center gap-3 flex-wrap">
                {[
                  { Icon: FaXTwitter, href: "https://x.com/chatzisocial" },
                  { Icon: FaInstagram, href: "https://www.instagram.com/chatzi_io/" },
                  { Icon: FaYoutube, href: "https://www.youtube.com/@chatzi.social" }
                ].map(({ Icon, href }, idx) => (
                  <a key={idx} href={href} target="_blank" rel="noopener noreferrer" className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#6119E6]/10 text-[#6119E6] dark:bg-[#E13382]/20 dark:text-[#E13382] transition hover:scale-105">
                    <Icon size={18} />
                  </a>
                ))}
              </div>
      ...
      <AnimatePresence>
        {showScrollTop && (
          <motion.button
            ...
            className="fixed bottom-6 end-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#6119E6] text-white shadow-xl transition hover:-translate-y-1 hover:bg-[#6119E6]/90 dark:bg-[#E13382] dark:hover:bg-[#E13382]/90"
